...
이 프로젝트는 **cmux-win**입니다. Windows용 AI 에이전트 터미널 멀티플렉서로, 여러 AI CLI(Codex, AGY, Codex 등)를 동시에 실행하고 협업시킬 수 있는 Electron 기반 데스크톱 앱입니다.
...
│ (200px)  │  (분할 가능, 드래그 재배치)          │
│──────────│                                      │
│ Explorer │  [Codex CLI] [AGY CLI] [Codex]  │
│ (파일)   │                                      │
│──────────│                                      │
│ 🧠Codex │                                      │
│ 💎AGY │                                      │
...
| CLI | 실행 방식 | 이유 |
|-----|----------|------|
| **Codex** | `Codex` (interactive) → `send_text`로 작업 지시 | TUI가 PTY write를 입력으로 받음 |
...
- "requires approval" → 자동 Enter (Codex)
- 1초 쿨다운으로 중복 방지
...
### 6. 브라우저 패널
- 사이드바에서 🧠Codex.ai / 💎AGY / 🤖ChatGPT 클릭 → 브라우저 패널 열기
- Codex.ai는 모바일 UA로 반응형 적용

### 7. 텔레그램 봇 연동
- 알림 전달 (outbound) + 원격 제어 (inbound)
- /status, /agents, /approve, /reject 명령
- /send agy "작업" — 특정 에이전트에 텍스트 전송
- /task "작업" — Codex 리더에게 작업 지시
- Bot token은 Electron safeStorage로 암호화 저장

### 8. Codex CLI 자동 실행
- 앱 시작 시 첫 번째 터미널에 Codex CLI 자동 실행
- Codex Desktop Dispatch 대상이 항상 존재
- 원격(핸드폰)에서 Dispatch → Codex CLI → tmux-shim → 다른 에이전트 제어
...
resources/
  bin/            — Codex.cmd, Codex-wrapper.js, tmux-shim.js
...
### tmux-shim 명령 (Codex Agent Teams 호환):
```bash
...
- Codex를 여러 패널에서 중복 실행하지 말 것 — 리더 1개만 실행
- /model 등 슬래시 명령은 해당 CLI 입력창에서만 동작 — send-keys로 전달 불가
- Codex.ai 웹은 데스크톱 반응형 CSS 없음 → 모바일 UA 적용
